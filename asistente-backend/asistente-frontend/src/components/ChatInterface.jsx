...
function ChatInterface({
  ...
  // Detectar tema de chat desde sessionStorage (viene de HealthPage)
  useEffect(() => {
    const chatTopic = sessionStorage.getItem('chatTopic');
    if (chatTopic && !inputValue) {
      setInputValue(chatTopic);
  ...
  return (
    <div className="flex flex-col h-full" style={{ minHeight: '100vh' }}>
      ...
      <div className="bg-gradient-to-r from-blue-600 via-blue-500 to-indigo-600 px-6 py-6 flex items-center justify-between shadow-lg">
        <div>
          <h1 className="text-3xl md:text-4xl font-bold text-white flex items-center gap-3">
            <span className="text-4xl md:text-5xl">📘</span>
            {userName ? `Hola, ${userName.split(' ')[0]} 👋` : 'Asistente'}
          </h1>
        ...
        <button
          ...
          className="bg-white/20 backdrop-blur-sm hover:bg-white/30 text-white px-6 py-4 rounded-2xl text-xl font-bold transition-all flex items-center gap-3 border border-white/30 shadow-lg"
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 pattern-bg">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center min-h-[500px] text-center px-4">
            ...
            <div className="bg-white rounded-3xl p-8 shadow-2xl max-w-2xl border-2 border-blue-100">
              <p className="text-gray-600 text-xl mb-6">
                Puedes hacer preguntas sobre cualquier tema:
              </p>
              <div className="grid gap-4 text-left">
                <button
                  onClick={() => setInputValue('¿Qué tiempo hace hoy?')}
                  className="text-left p-4 bg-blue-50 hover:bg-blue-100 rounded-2xl transition-colors text-blue-700 text-lg font-semibold"
                >
                  🌤️ ¿Qué tiempo hace hoy?
                </button>
                <button
                  onClick={() => setInputValue('Explícame cómo usar WhatsApp')}
                  className="text-left p-4 bg-green-50 hover:bg-green-100 rounded-2xl transition-colors text-green-700 text-lg font-semibold"
                >
                  📱 Explícame cómo usar WhatsApp
                </button>
                <button
                  onClick={() => setInputValue('¿Cuáles son las noticias de hoy?')}
                  className="text-left p-4 bg-purple-50 hover:bg-purple-100 rounded-2xl transition-colors text-purple-700 text-lg font-semibold"
                >
                  📰 ¿Cuáles son las noticias de hoy?
                </button>
                <button
                  onClick={() => setInputValue('¿Qué ejercicios puedo hacer en casa?')}
                  className="text-left p-4 bg-orange-50 hover:bg-orange-100 rounded-2xl transition-colors text-orange-700 text-lg font-semibold"
                >
                  🏋️ ¿Qué ejercicios puedo hacer en casa?
                </button>
              </div>
            </div>
